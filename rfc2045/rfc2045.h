...
namespace rfc6533 {
	...
	// RFC 6533 encoding format

	enum class format {
		utf_8,		// Native UTF-8

		xtext,		// utf8-add-xtext

		unitext,	// utf-8-addr-unitext

		rfc822		// RFC-822, and convert domain to ACE, too
	};

	// Use RFC 6533 to encode a UTF-8 address.
	//
	// The E-mail address is encoded as "rfc822" address type if it has only
	// ASCII characters and format is utf_8, otherwise the format parameter
	// specifies the "utf-8" encoding format.

	std::string encode(std::string_view address, format f);

	// Overload for a u32string_view
	std::string encode(std::u32string_view address, format f);

	// Decode a utf-8 or an rfc-822 address type.
	//
	// Assumes valid UTF-8 coding, and does not verify it.
	//
	// Does verify, for both rfc-822 and utf-8 formats, that the returned
	// address does not contain control characters.
...
namespace rfc2045 {
	...
	// MIME decoder might emit the decoded chunk in small pieces. To be
	// efficient we'll combine them into bigger chunk. Take the real
	// output sink, and present an output sink that chunks things in
	// BUFSIZ-sized bytes.

	template<typename out_iter>
	struct chunking_decoder {
		out_iter &real_out;
		char buf[BUFSIZ];
		size_t s{0};
	public:
		chunking_decoder(out_iter &real_out) : real_out{real_out}
		{
		}
...
)
{
	...
	chunking_decoder chunk{out};

	if (charset.empty() || std::string_view{
			e.content_type.value
		}.substr(0, 5) != "text/")
	{
		if (!e.decode_body(src, chunk))
			decoding_error=true;
	}
	else
	{
		rfc2045::entity::converter converter{chunk};
